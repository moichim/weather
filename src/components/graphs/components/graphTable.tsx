...
export const GraphTable: React.FC<GraphInstanceWithDataPropsType> = props => {

    const { stack } = useGraphContext();

    if (stack.state.isSelecting) return <></>

    if (!stack.state.selectionStart && !stack.state.selectionEnd) return <></>
    ...
    return <Table hideHeader cellPadding={2}>
        ...
        <TableBody>
            ...
            {props.data.tableData.map(row => <TableRow key={row.name} style={{ color: row.color }} aria-labelledby={row.name}>
